...
function renderSymbolCall(toolName: string) {
  return (args: Record<string, unknown>, theme: any, context: any) => {
    ...
    if (args.name)
      content += " \u2014 " + theme.fg("accent", String(args.name));
    if (args.path)
      content += "  " + theme.fg("muted", "in " + String(args.path));
    if (args.kind)
      content += "  [" + theme.fg("dim", "kind: " + String(args.kind)) + "]";
...
export default async function (pi: ExtensionAPI) {
    ...
    if (isToolCallEventType("edit", event)) {
      ...
      try {
        ...
        if (err) {
          ...
          const cause = leftover
            ? describePrefixLeftover(leftover) + "\n\n"
            : "";
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      path: Type.Optional(
        Type.String({
          description:
            "File path to list symbols from. Omit to list across all project files.",
        }),
      ),
      kind: Type.Optional(
        Type.String({
          description:
            "Filter by symbol kind: function, class, method, interface, type, variable",
        }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    renderResult: renderSymbolResult(),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      const allResults = await extractAllFiles(
        process.cwd(),
      ...
      if (hits.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: "No definition found for '" + params.name + "'",
            },
          ],
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Find all call sites of a function or method across the project",
    promptGuidelines: [
      ...
      if (callers.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: "No callers found for '" + params.name + "'",
            },
          ],
  ...
  pi.registerTool({
    ...
    renderResult(result, { expanded }, theme) {
      if (result.isError || !result.details?.body) {
        return new Text(
          theme.fg("error", result.content?.[0]?.text || "Error"),
          0,
          0,
        );
      }
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (!extracted) {
        return {
          content: [{ type: "text", text: "Could not parse " + filePath }],
          details: {},
        };
      }
      for (const sym of extracted.symbols) {
        if (sym.name === params.name) {
          ...
          if (source === null) {
            return {
              content: [{ type: "text", text: "Could not read " + filePath }],
              details: {},
            };
          }
      ...
      return {
        content: [
          {
            type: "text",
            text: "Symbol '" + params.name + "' not found in " + filePath,
          },
        ],
        details: {},
      };
      ...
      if (!ext)
        return {
          content: [
            {
              type: "text",
              text: "No callees found for '" + params.name + "'",
            },
          ],
